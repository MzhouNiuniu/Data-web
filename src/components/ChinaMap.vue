...
<script>
    ...
    export default {
        ...
        computed: {
            isRoot() {
                return this.blockStack.length <= 1;
            },
            currentBlock() {
                return this.blockStack[this.blockStack.length - 1];
            },
            currentBlockProps() {
                return this.currentBlock.properties;
            },
            currentBlockName() {
                return this.currentBlockProps.name;
            },
            currentBlockCode() {
                return this.currentBlockProps.adcode;
            },
        },
        methods: {
            // 工具
            ...
            checkIsLastBlock(block) {
                // 包括 childrenNum === 0、不存在
                return !block.properties.childrenNum;
            },
            updateMap() {
            ...
            // 外部使用：对外提供一个方法，一般用不到；
            // 内部使用：一般都是修改blockStack之后更新地图，所以使用this.currentBlockCode作为code默认值
            loadGeoData(code = this.currentBlockCode) {
            ...
            pushStack(block) {
                this.isLastLevel = this.checkIsLastBlock(block);
            ...
            cleanStack() {
                ...
                this.blockStack.push(1), this.blockStack.pop(); // 触发computed更新 + $forceUpdate()
            },
            handleChange() {
                this.$emit('change', this.blockStack);
            },
            ...
            handleMapSelectChange(params) {
                ...
                // 获取地图
                const currentBlock = echarts.getMap(this.currentBlockCode);
                const targetBlock = currentBlock.geoJson.features.find(item => item.properties.name === triggerBlock.name);
                if (!targetBlock) {
                    return;
                }

                this.pushStack(targetBlock);
                this.handleChange();
                this.loadGeoData();
            },
            ...
            async jumpTo(nameStack) {
                ...
                for (let i = 0; i < nameStack.length; i++) {
                    let nextBlock = target.features.find(item => item.properties.name === nameStack[i]);
                    if (!nextBlock) {
                        return;
                    }

                    this.pushStack(nextBlock);

                    if (i === nameStack.length - 1) {
                        this.handleChange();
                    }

                    target = await this.loadGeoData();
                }
            },
            reset() {
                this.cleanStack();
